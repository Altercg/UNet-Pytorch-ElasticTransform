...
class Unet(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.up6 = nn.ConvTranspose2d(1024, 512, 2, stride=2)   # 逆卷积
    ...
    def forward(self, x):
        ...
        c5 = self.conv5(p4)
        up_6 = self.up6(c5)
        merge6 = torch.cat([up_6, transform4(c4)], dim=1)   # 横向合并

        c6 = self.conv6(merge6)
        up_7 = self.up7(c6)
        merge7 = torch.cat([up_7, transform3(c3)], dim=1)

        c7 = self.conv7(merge7)
        up_8 = self.up8(c7)
        merge8 = torch.cat([up_8, transform2(c2)], dim=1)

        c8 = self.conv8(merge8)
        up_9 = self.up9(c8)
        merge9 = torch.cat([up_9, transform1(c1)], dim=1)
